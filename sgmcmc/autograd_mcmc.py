...
class ResetSampler(SGMCMCSampler):
    ...
    def step(self, *inputs):
        ...
        if self.precondition and self.noise_correction:
            ...
            if self.precondition:
                ...
                noise = Minv**2 * 0.5 * self.epsilon
        # pre-generate a sample
        eps = self.epsilon * np.exp(-0.01 * (self.count - self.base_count))
        if self.count % 100 == 0:
            self.base_count = self.count
        sample = 0. #self._srng.normal(size=self.theta.shape) * np.sqrt(eps * 2 * (self.A - noise) * Minv)
        if self.resample_momentum > 0 and self.count % self.resample_momentum == 0:
            # optionally resample momentum 
            np.copyto(self.updates, self._srng.normal(size=self.theta.shape))
        else:
            # compute updates
            self.updates = - eps * self.A * Minv * grad + sample + eps * gMinv
        # and apply 
        self.theta += self.updates
